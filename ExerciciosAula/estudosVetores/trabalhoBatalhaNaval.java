...
public class trabalhoBatalhaNaval{
    public static void main(){
        ...
        /*apenas para testar quais foram sorteados:*/ imprimirMatriz(matriz);
        int placar1 = 0;
        int placar2 = 0;
       
        while(placar1 != 5 || placar2 != 5){
            if(acertouJogador1(matriz, jogador1) == true){
                placar1++;
                System.out.printf("Parabéns %s, você acertou, sua pontuação é de %d ponto(s)", jogador1, placar1);
            }
            else if(acertouJogador1(matriz, jogador1) == false){
                placar1++;
                System.out.printf("%s, você errou, sua pontuação é de %d ponto(s)", jogador1, placar1);
            }
            ...
            else if(acertouJogador2(matriz, jogador2) == false){
                placar2++;
                System.out.printf("%s, você errou, sua pontuação é de %d ponto(s)", jogador2, placar2);
            }
        }
    ...
    //MÉTODO PARA PREVENIR REPETIÇÃO NA MATRIZ
    ...
    public static boolean acertouJogador1(int[][]m, String j1){
        ...
        System.out.printf("%n%s, digite um número que acha que contém na matriz: ", j1);
        int n = in.nextInt();
        for(int lin = 0; lin<m.length; lin++){
            for(int col = 0; col<m[0].length; col++){
                if(n == m[lin][col]){
